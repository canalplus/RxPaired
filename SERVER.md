...
- **Initial Device Message**: Message sent by the server which describes the
  ...
  - `value` (object): Object with the following keys:
    ...
    - `version` (number, optional): `2` when the device declared the signed
      app timestamp offset format. Absent for older devices.

    - `time` (string, optional): `"capture"` for version 2.

    - `offset` (string, optional): `"app-capture"` for version 2.
...
- **logs**: Other messages that do not correspond to either of the preceding
  ...
  They should be encoded as a UTF-8 string in the format
  `<TIMESTAMP><SIGNED_APP_OFFSET> [<LOG_NAMESPACE>] <LOG_MESSAGE>`.
  Omit `<SIGNED_APP_OFFSET>` when unavailable or zero after rounding.
  Examples with and without that offset:

  ```text
  12500.44-850.25 [info] Delayed worker log
  12500.45 [info] Ordinary log
  ```
  ...
  - `<SIGNED_APP_OFFSET>` is optional. When present, it is the signed
    difference `app timestamp - TIMESTAMP` in milliseconds (for example,
    `-850.25`). It is omitted when it rounds to zero.
  ...
  - `<LOG_NAMESPACE>` is an identifier for the type of log. It is generally
    ...
    for Network-related log). Its square brackets are literal characters in
    the log; the optional offset has no brackets.
...
- **Initial Message**: This should be the initial message sent by the
  ...
  format:
  `Init v2 <TIMESTAMP> <CURRENT_DATE> time=capture offset=app-capture`

  Older devices may still send `Init v1 <TIMESTAMP> <CURRENT_DATE>`.
...
- **logs**: Other messages that do not correspond to either of the preceding
  ...
  - `<SIGNED_APP_OFFSET>` is optional. When present, it is the signed
    difference `app timestamp - TIMESTAMP` in milliseconds and is omitted
